    ...
    with open(fname) as f:
        ...
        for line in f.readlines():
            ...
            if len(cols) > 1:
                ...
                word = NUM.sub('0', word)
                tags = dict()
                for name, tag in zip(feat_list, cols[2:]):
                    tags[name] = tag
...
class Tagger(dy.Saveable):
    def __init__(self, n_words, n_chars, n_w_emb, n_c_emb, n_d_emb, n_hidden,
                 n_layer, n_mlp, vw, vc, vts, dict_feats, feat_dict, optimizer, feature):
        self.model = dy.Model()
        ...
        if args.use_dict:
            self.FEAT_LOOKUPs = {k: self.model.add_lookup_parameters((dict_feats[k].size(),\
                                 n_d_emb)) for k,v in dict_feats.items()}
            if args.dict_all:
    ...
    def train(self, train, dev, n_epoch, path):
        ...
        for ITER in range(n_epoch):
            ...
            for s in train:
                ...
                i += 1
                sys.stderr.write("\rTraining:\t\033[K" + get_progressbar_str(progress))
                sys.stderr.flush()
            sys.stderr.write("\n")
    ...
    def accuracy(self, corpus):
        ...
        for sent in corpus:
            ...
            i += 1
            sys.stderr.write("\rPredicting:\t\033[K" + get_progressbar_str(progress))
            sys.stderr.flush()
        sys.stderr.write("\n")
...
def train_mode():
    ...
    if args.use_dict:
        if not os.path.exists("log/UD_separate_dict"):
            os.mkdir("log/UD_separate_dict")
        log_dir = os.path.join("log/UD_separate_dict", feature+ "_" +\
                               args.optimizer+now)
    else:
        if not os.path.exists("log/UD_separate"):
            os.mkdir("log/UD_separate")
        log_dir = os.path.join("log/UD_separate", feature+ "_" +\
                               args.optimizer+now)
    os.mkdir(log_dir)
    ...
    tagger = Tagger(n_words=n_words, n_chars=n_chars,
                    ...
                    vts=vts, dict_feats=dict_feats, feat_dict=feat_dict, optimizer=args.optimizer,
                    feature=feature)
...
def test_mode():
    ...
    tagger = Tagger(n_words=n_words, n_chars=n_chars,
                    ...
                    optimizer=args.optimizer, feature=feature)
